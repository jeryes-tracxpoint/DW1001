...
DW1000Mac    DW1000Class::_globalMac1;
bool DW1000Class::IsIdleDone = 1;

boolean DW1000Class::_enableRcvFaild = false;
...
float DW1000Class::AntenaDelayDevice1=16254;//16655;//15904;//16655;//16384;
...
byte 	   DW1000Class::_PositionXYZ[3];
byte 	   DW1000Class::data1[30];

// monitoring
byte DW1000Class::_vmeas3v3 = 0;
byte DW1000Class::_tmeas23C = 0;
uint8_t  DW1000Class::FillterSyncNumber =1;
...
//void    DW1000Class::SetDeviceMode(uint8_t CurMode);
...
void DW1000Class::select(uint8_t ss) {
	...
	#ifdef DOUBLE_BUFFER
		setDoubleBuffering(DBLBUF);
		setReceiverAutoReenable(AUTRCV);
		setBit(_sysmask, LEN_SYS_MASK, RXOVRR, DBLBUF);
	...
	_vmeas3v3 = buf_otp[0];
	//Serial.println("_vmeas3v3:");Serial.println(_vmeas3v3);
	readBytesOTP(0x009, buf_otp); // the stored 23C reading
	_tmeas23C = buf_otp[0];
	//Serial.println(",_tmeas23C:");Serial.println(_tmeas23C);
}
...
void DW1000Class::begin(uint8_t irq, uint8_t rst) {
	// generous initial init/wake-up-idle delay -delay(ms)
	delay(5);
...
void DW1000Class::manageLDE() {
	...
	if(ldoTune[0] != 0) {
		...
		//Serial.print("ldoTune[0]= ");Serial.println(ldoTune[0]);
	}
...
void DW1000Class::enableClock(byte clock) {
	...
	byte pmscctrl0[LEN_PMSC_CTRL0];
	memset(pmscctrl0, 0, LEN_PMSC_CTRL0);
	//PMSC = Power managment and system control
	//PMSC_CTRL0_SUB = control regester zero
	readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
	if(clock == AUTO_CLOCK) {
...
void DW1000Class::matheConstantPara(uint8_t FillterSync ){

	FillterSyncNumber = FillterSync;
}
...
void 	DW1000Class::PrintANchorRCVData(){
	uint16_t BlinkNumberr = 0;
	DW1000.getData(data1,30);
	bool ToRecv = false;
	...
	if (ToRecv){
		...
		BlinkNoUnion.BlinkNoByte[0] = data1[21];
		BlinkNoUnion.BlinkNoByte[1] = data1[22];



	BlinkNumberr =BlinkNoUnion.BlinkNoUint16;// data1[22]*256+data1[21];

	#ifdef FLTR

	Serial.print("BlinkN: ");
	Serial.print(BlinkNumberr);

	Serial.print(",Tag No: ");
	Serial.print(data1[13]);Serial.print(":");Serial.print(data1[14]);Serial.print(":");Serial.print(data1[15]);Serial.print(":");
	Serial.print(data1[16]);Serial.print(":");Serial.print(data1[17]);Serial.print(":");Serial.print(data1[18]);Serial.print(":");
	Serial.print(data1[19]);Serial.print(":");;Serial.print(data1[23]);

	Serial.print(",Is Sync: ");
	Serial.print(data1[24]);
		
	int64_t TIME_OVERFLOW1 = 0x10000000000; 
	float TIME_RES1     = 0.000015650040064103f;
	float TIME_RES_INV = 63897.6f;
	int64_t TagSyncTimeLocal ;	  
	uint64_t TagTimeInAnchor=0;
	...
	floatAsBytes.bval[0] = data1[25];
	floatAsBytes.bval[1] = data1[26];
	floatAsBytes.bval[2] = data1[27];
	floatAsBytes.bval[3] = data1[28];
	floatAsBytes.bval[4] = data1[29];

	float TagSyncTime = 	(floatAsBytes.fval%TIME_OVERFLOW1)*TIME_RES1;
	TagSyncTimeLocal = (int64_t)(TagSyncTime*TIME_RES_INV);
	byte address[8];
	byte shortAddress[2];

	// we get the long address,shortaddress of the Blink sender 
	_globalMac1.decodeBlinkFrame(data1, address, shortAddress);


	DW1000Device myTag(address, shortAddress);

	DW1000Device* myDistantDevice =&myTag;//searchDistantDevice(address);
	DW1000.getReceiveTimestamp(myDistantDevice->timeBlinkReceived);
			
	myDistantDevice ->timeSync.setTime(TagSyncTime);

	Serial.print(",Tag time: ");myDistantDevice->timeSync.print();

	Serial.print(",Anchor time: ");myDistantDevice->timeBlinkReceived.print();
	Serial.print(",RF: LOS");  
	char msg[90];
	PrintDeviceIdTowByte(msg);		
	Serial.print(",Device ID: ");
	Serial.print(msg);
	byte PPindx[2];
	DW1000.getLEdPPIndx(PPindx);
	...
	Serial.print(getReceiveQuality() );
	Serial.print(",PIndx: ");
	Serial.println(getPeakPathIndx() );
	...
	Serial.print(",");myDistantDevice->timeSync.print();

	Serial.print(",");myDistantDevice->timeBlinkReceived.print();
	...
	//Serial.print(RXP - FP );
	Serial.print(",");
	Serial.println(getReceiveQuality() );
	//Serial.print(",");
	//Serial.println(getPeakPathIndx() );
	#endif
...
void DW1000Class::handleInterrupt() {
	...
	readSystemEventStatusRegister();
			...
			boolean valHSRBP  = getBit(_sysstatus, LEN_SYS_STATUS, HSRBP_BIT);
	 		boolean valICRBP  = getBit(_sysstatus, LEN_SYS_STATUS, ICRBP);
			

			if (valICRBP != valHSRBP){
			...
			//Serial.print("aftre Read fropm valHSRBP:");Serial.println(getBit(_sysstatus, LEN_SYS_STATUS, HSRBP_BIT));

			if (getBit(_sysstatus, LEN_SYS_STATUS, RXFCG_BIT)){
				PrintANchorRCVData();
	
			}
			readSystemEventStatusRegister();
			//Serial.print(",RXOVRR:");Serial.print(getBit(_sysmask, LEN_SYS_MASK, RXOVRR));
			if (getBit(_sysmask, LEN_SYS_MASK, RXOVRR)){
				Serial.println(",Bug");
			}
			//valHSRBP  = getBit(_sysstatus, LEN_SYS_STATUS, HSRBP_BIT);
	 		//valICRBP  = getBit(_sysstatus, LEN_SYS_STATUS, ICRBP);
	...
	if(isReceiveFailed() && _handleReceiveFailed != 0) {
		...
		Serial.println("isReceiveTimeout");
	...
	//	(*_handleReceived)();
		DW1000.getData(data1,30);
		Serial.print("ID:");Serial.println(data1[23]);
		clearReceiveStatus();
		if(_permanentReceive) {
			newReceive();
			startReceive();
...
void DW1000Class::PrintDeviceIdTowByte(char msgBuffer[]){
	byte data[LEN_EUI];
	readBytes(EUI, NO_SUB, data, LEN_EUI);
	sprintf(msgBuffer, "%02X:%02X",
					data[7], data[6]);
}
...
void DW1000Class::mask(){

			setBit(_sysmask, LEN_SYS_STATUS, RXFCE_BIT, MSK);
			setBit(_sysmask, LEN_SYS_STATUS, RXFCG_BIT, MSK);
			setBit(_sysmask, LEN_SYS_STATUS, RXDFR_BIT, MSK);
			setBit(_sysmask, LEN_SYS_STATUS, LDEDONE_BIT, MSK);
			writeBytes(SYS_MASK, NO_SUB, _sysmask, LEN_SYS_MASK);

}

void DW1000Class::Unmask(){
...
void DW1000Class::setDeviceAddress(uint16_t val) {
	...
	Serial.print("_networkAndAddress[0]= ");Serial.println(_networkAndAddress[0]);

	_networkAndAddress[1] = (byte)((val >> 8) & 0xFF);
	Serial.print("_networkAndAddress[1]= ");Serial.println(_networkAndAddress[1]);
}

void DW1000Class::setPositionXYZ(uint16_t val){

	_PositionXYZ[1] = (byte)(val & 0xFF);
	_PositionXYZ[2] = (byte)((val >> 8) & 0xFF);
	_PositionXYZ[3] = (byte)((val >> 16) & 0xFF);
}
...
void DW1000Class::getLEdPPIndx(byte PPIndx){

	byte PPIndxByte[2];
//	readBytes(LED_INF, LED_PPINDX_SUB, PPIndx, 2);
...
void DW1000Class::getTempAndVbat(float& temp, float& vbat) {
	// follow the procedure from section 6.4 of the User Manual (page 62 and 158)
...
void DW1000Class::startReceive() {
		...
		boolean valHSRBP  = getBit(_sysstatus, LEN_SYS_STATUS, HSRBP_BIT);
		boolean valICRBP  = getBit(_sysstatus, LEN_SYS_STATUS, ICRBP);
		//setBit(_sysstatus, LEN_SYS_STATUS, ICRBP, !valICRBP);

		/* if (valICRBP != valHSRBP){
			setBit(_sysctrl, LEN_SYS_CTRL, HRBPT_BIT, valICRBP);
			Serial.println("SHONEMM");
		} */
...
void DW1000Class::commitConfiguration() {
	...
	//flast value (third anchor ) writeValueToBytes(antennaDelayBytes, 146846, LEN_STAMP);
	//second anchor writeValueToBytes(antennaDelayBytes, 147380, LEN_STAMP);
	writeValueToBytes(antennaDelayBytes, AntenaDelayDevice1, LEN_STAMP);
...
void DW1000Class::getReceiveTimestamp(DW1000Time& time) {
	...
	// jeryes comment this function to check the the time for Tdoa ranging
...
void DW1000Class::clearDBLBuff(){

	setBit(_sysstatus, LEN_SYS_STATUS, RXFCE_BIT, true);
	
	setBit(_sysstatus, LEN_SYS_STATUS, RXFCG_BIT, true);
	setBit(_sysmask, LEN_SYS_STATUS, RXDFR_BIT, true);
	setBit(_sysmask, LEN_SYS_STATUS, LDEDONE_BIT, true);
	writeBytes(SYS_STATUS, NO_SUB, _sysstatus, LEN_SYS_STATUS);
}
...
float DW1000Class::getPeakPathIndx(){

	byte         LDEByte[2];
	byte         LedByte2[2];

	readBytes(LED_INFF,LED_PPINDX_SUB,LDEByte,2);
	readBytes(LED_INFF,LED_PPAMPL_SUB,LedByte2,2);
	
	uint16_t noise1 = (uint16_t)LDEByte[0] | ((uint16_t)LDEByte[1] << 8);
	uint16_t f = (uint16_t)LedByte2[0] | ((uint16_t)LedByte2[1] << 8);
	...
	return float(noise1/estFpPwr);

}


byte DW1000Class::getModeDevice(int indx){

	if (indx ==0 ){
		return MODE_LONGDATA_RANGE_LOWPOWER;
	}else if (indx == 5 ){
		return MODE_LONGDATA_RANGE_ACCURACY;
	}else{
		return MODE_LONGDATA_RANGE_ACCURACY;
	}
}

//get the first path power see page 102 in user manual
...
void DW1000Class::WriteBytesOTP(uint16_t address, byte data[]) {
	...
	//writeBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, 2);
	writeByte(PMSC, PMSC_CTRL0_SUB, 0x01); //
	writeByte(OTP_IF, OTP_CTRL_SUB, 0x03);
	writeByte(OTP_IF, OTP_WDAT_SUB, 0x9220);
	writeByte(OTP_IF, OTP_WDAT_SUB, 0x9220);
	writeByte(OTP_IF, OTP_CTRL_SUB, 0x08);
	delay(1000);


	writeByte(OTP_IF, OTP_CTRL_SUB, 0x00); // OTPRDEN | OTPREAD


	writeBytes(OTP_IF, OTP_WDAT_SUB, data, 4);


	//writeBytes(OTP_IF, OTP_WDAT_SUB, data);
	writeBytes(OTP_IF, OTP_ADDR_SUB, addressBytes, 2);
	writeByte(OTP_IF, OTP_CTRL_SUB, 0x40); // OTPRDEN | OTPREAD
	writeByte(OTP_IF, OTP_CTRL_SUB, 0x00); // OTPRDEN
...
//read two byte from LED regester to get the data from acculmater of DW1000

void DW1000Class::readLEDPPINDX(uint16_t address, byte data[]){
